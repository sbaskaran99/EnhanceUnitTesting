...
openai_agent = OpenAIWrapper(api_key=os.getenv("OPENAI_API_KEY"),model="gpt-4")
#openai_agent = OpenAIWrapper(model="gpt-4")
...
# Fix failing test using OpenAI GPT-4
def fix_test_case(test_function_code, error_reason):
    ...
    response = openai_agent.complete(prompt=prompt)
